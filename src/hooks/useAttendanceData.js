...
export const useAttendance = (page = 1, limit = 10) => {
  ...
  const fetchAttendanceRecords = async (pageNum = currentPage, pageLimit = pageSize) => {
    try {
      setLoading(true);
      setError(null);
('🔍 useAttendance: Fetching all attendance records for client-side pagination');
...
('🔍 useAttendance: API response:', data);
...
('🔍 useAttendance: Total records fetched:', allRecords.length);
...
('Error fetching attendance records:', err);
...
('🔍 Max pages:', maxPages, 'Total records:', totalRecords, 'Page size:', pageSize);
...
('🔍 prevPage called, current page:', currentPage);
  ...
  const deleteAttendance = async (id) => {
    try {
('Hook: Deleting attendance record with ID:', id);
      await attendanceApi.deleteAttendance(id);
('Hook: Delete successful, refreshing attendance records...');
      // Refresh the attendance records after deletion
      await fetchAttendanceRecords();
('Hook: Attendance records refresh completed');
    } catch (err) {
('Hook: Delete attendance error:', err);
