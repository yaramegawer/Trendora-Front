...
import { useITLeaves } from "../../hooks/useITData";
import { useNotification } from "../../contexts/NotificationContext";
import SimplePagination from "../common/SimplePagination";

const ITLeavesManagement = () => {
  ...
  // Use the IT leaves hook with pagination
  const {
    leaves,
    ...
    goToPage: leavesGoToPage,
    changePageSize: leavesChangePageSize,
    updateLeaveStatus,
    deleteLeave,
  } = useITLeaves(leavesCurrentPage, pageSize);

  // When searching or filtering by status, fetch a large page to cover all items
  useEffect(() => {
    const needsAll = statusFilter !== "all" || (searchTerm && searchTerm.trim().length > 0);
    if (needsAll) {
      setLeavesCurrentPage(1);
      if (typeof leavesChangePageSize === "function") {
        leavesChangePageSize(1000);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [statusFilter, searchTerm]);

  // Also fetch a large page on mount to ensure we have all leaves for client-side pagination
  useEffect(() => {
    if (typeof leavesChangePageSize === "function") {
      leavesChangePageSize(1000);
    }
  ...
  // Filter leaves based on status and search (client-side) only in local mode
  ...
  const handleStatusFilterChange = (newStatus) => {
    ...
    setLeavesCurrentPage(1); // Reset to first page
  };
    ...
    <div>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ color: "#6b7280", fontSize: "14px" }}>
          Manage and review leave requests from IT department employees
        </p>
      </div>
      ...
      {!leavesLoading && !leavesError && (
        <>
          ...
          {/* Pagination */}
          {(isLocalMode ? filteredLeaves.length > 0 : (leaves || []).length > 0) && (
            <div style={{ marginTop: "24px" }}>
              {isLocalMode ? (
                ...
                <SimplePagination
                  ...
                  onPageChange={(page) => {
                    setLeavesCurrentPage(page);
                    if (typeof leavesGoToPage === "function") {
                      leavesGoToPage(page);
                    }
                  }}
...
export default ITLeavesManagement;
