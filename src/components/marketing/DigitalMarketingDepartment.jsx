...
import MarketingLeavesManagement from "./MarketingLeavesManagement";
import MarketingDepartmentWrapper from "./MarketingDepartmentWrapper";
...
const DigitalMarketingDepartment = () => {
  ...
  useEffect(() => {
    ...
    if (employees.length === 0 && !employeesLoading) {
      // Silent: No employees found for Digital Marketing department
    }
  ...
  const [customerProjectsCurrentPage, setCustomerProjectsCurrentPage] =
    useState(1);
  ...
  const [newProjectFieldErrors, setNewProjectFieldErrors] = useState({
    name: "",
    description: "",
  });
  ...
  const [newProjectDateErrors, setNewProjectDateErrors] = useState({
    start: "",
    end: "",
    range: "",
  });
  const [editProjectDateErrors, setEditProjectDateErrors] = useState({
    start: "",
    end: "",
    range: "",
  });
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !event.target.closest(".project-status-dropdown-container")
      ) {
        setShowProjectStatusDropdown(false);
      }
    };
  ...
  const fetchCustomers = async (page = 1, limit = 10) => {
    try {
      ...
      if (Array.isArray(customersData)) {
        ...
        totalCustomers =
          customersData.total ||
          customersData.totalPages * limit ||
          customersData.data.length;
        "📊 Using backend pagination data:",
          {
      ...
      // Silent: Error fetching customers
      setCustomers([]);
      setCustomersTotal(0);
    } finally {
  ...
  const fetchCustomerProjects = async (
    ...
    try {
      ...
      "🔄 Fetching projects for customer:",
        customerName,
        "Page:",
        page,
        "PageSize:",
      ...
      "🔄 API Endpoint will be:",
        `/digitalMarketing/customers/${customerName}/projects`;

      // Pass status filter to API
      const projectsData = await marketingCustomerApi.getCustomerProjects(
        customerName,
        page,
        pageSize,
        status
      ...
      if (Array.isArray(projectsData)) {
        ...
        totalProjects =
          projectsData.total ||
          projectsData.totalPages * pageSize ||
      ...
      "✅ State updated - customerProjects:",
        projectsList.length,
  ...
  const submitRatingWithNote = async (employeeId, note = "") => {
    ...
    if (typeof updateRating !== "function") {
      // Silent: updateRating is not a function
      showError(
        "Error: Rating function not available. Please refresh the page."
      );
      return;
    }
    ...
    const efficiency = Math.max(
      employeeRatings[`${employeeId}-efficiency`] || 1,
      1
    );
    const performance = Math.max(
      employeeRatings[`${employeeId}-performance`] || 1,
      1
    );
    const teamwork = Math.max(
      employeeRatings[`${employeeId}-teamwork`] || 1,
      1
    );
    ...
    try {
      ...
      const ratingData = {
        ...
        note:
          note ||
          `Rating updated - Performance: ${performance}, Efficiency: ${efficiency}, Teamwork: ${teamwork}`,
      };
  ...
  const handleCreateProject = async () => {
    ...
    // Description is optional; if provided, enforce min length
    ...
    if (newProject.endDate) {
      ...
      if (isNaN(endDate.getTime())) {
        setNewProjectDateErrors((prev) => ({
          ...prev,
          end: "Please enter a valid end date",
        }));
        return;
      }
    }
    ...
    try {
      ...
      // Include description only if non-empty to respect optional nature
      if (trimmedDescription) {
        projectData.description = trimmedDescription;
      }
      ...
      try {
        ...
        if (
          ...
          customerNameError.message.includes("customerName") &&
          customerNameError.message.includes("not allowed")
        ) {
          ...
          if (trimmedDescription) {
            fallbackProjectData.description = trimmedDescription;
          }

          await createProject(fallbackProjectData);
          showSuccess(
            "Project created successfully! (Note: Customer name field not supported by backend)"
          );
          setNewProject({
            name: "",
            description: "",
    ...
    try {
      ...
      if (!objectIdRegex.test(projectId)) {
        ...
        const validId = possibleIds.find((id) =>
          objectIdRegex.test(String(id).trim())
        );
        if (validId) {
          projectId = String(validId).trim();
        } else {
          throw new Error(
            `Invalid project ID format: "${projectId}". Expected 24-character hex string.`
          );
        }
      }
        ...
        if (editingProject.hasOwnProperty(field)) {
          if (field === "members") {
            ...
            if (editingProject.members && editingProject.members.length > 0) {
              ...
              const memberIds = editingProject.members
                .map((member) => {
      ...
      const hasStart =
        ...
        String(updateData.startDate).trim() !== "";
      const hasEnd =
        updateData.endDate !== undefined &&
        String(updateData.endDate).trim() !== "";
      if (hasStart) {
      ...
      // Silent: Error updating project

      // If customerName field is not allowed, retry without it
      if (
        error.message &&
        error.message.includes("customerName") &&
        ...
        try {
          ...
          if (fallbackUpdateData.notes !== undefined) {
            if (
              fallbackUpdateData.notes.length > 0 &&
              fallbackUpdateData.notes.length < 5
            ) {
              throw new Error(
                "Notes must be at least 5 characters or left empty"
              );
            }
          }

          "📤 Fallback update data:",
            JSON.stringify(fallbackUpdateData, null, 2);

          await updateProject(projectId, fallbackUpdateData);
          showSuccess(
            "Project updated successfully! (Customer name stored in notes)"
          );
          setShowEditProject(false);
          setEditingProject(null);
          setIsEditNewCustomer(false);
          ...
          // Silent: Error updating project (fallback)
    ...
    >
      <div style={{ padding: "32px" }}>
            ...
            >
              ...
              <div
                style={{
                  padding: "10px",
                  background:
                    "linear-gradient(135deg, #1c242e 0%, #334155 100%)",
                  borderRadius: "10px",
                }}
              >
          ...
          <div style={{ padding: "24px" }}>
            <div style={{ marginBottom: "24px" }}>
              ...
              <p style={{ color: "#6b7280", fontSize: "16px" }}>
                Overview of digital marketing activities and campaign
                performance
              </p>
            </div>
                    ...
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      <div style={{ marginBottom: "12px" }}>
                        ...
                        <input
                          ...
                          style={{
                            ...
                            height: "4px",
                            background:
                              "linear-gradient(to right, #e5e7eb 0%, #e5e7eb 60%, #e5e7eb 100%)",
                            outline: "none",
                            borderRadius: "2px",
                            cursor: "pointer",
                          }}
                      ...
                      <div style={{ marginBottom: "12px" }}>
                        ...
                        >
                          ...
                          >
                            {getRating(employee.id || employee._id, "teamwork")}
                            /5
                          </span>
                        </div>
                        <input
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button
                        ...
                        onClick={async (e) => {
                          ...
                          // Get current ratings for this employee
                          const efficiency = getRating(
                            employeeId,
                            "efficiency"
                          );
                          const performance = getRating(
                            employeeId,
                            "performance"
                          );
                          const teamwork = getRating(employeeId, "teamwork");
                          ...
                          try {
                            const note =
                              employeeRatings[`${employeeId}-note`] || "";

                            // Validate rating values
                            if (!efficiency || !performance || !teamwork) {
                              showWarning(
                                "Please provide all rating values before submitting."
                              );
                              return;
                            }
                            ...
                            // Silent: Failed to submit rating
                            showError(
        ...
        {activeTab === "leaves" && (
          ...
          >
            {/* Pass Digital Marketing department id for department-scoped leaves */}
            <MarketingLeavesManagement
              departmentId={"68da6e0813fe176e91aefd59"}
            />
          </div>
        )}
          ...
          >
                  ...
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleProjectSearchChange(e.target.value)
                        }
                        placeholder={
                          showCustomerSections
                            ? "Search projects..."
                  ...
                  {showCustomerSections && (
                    <div style={{ flex: 1, minWidth: "200px" }}>
                      ...
                      >
                        ...
                        <div
                          className="project-status-dropdown-container"
                          style={{ position: "relative", width: "100%" }}
                        >
                          <div
                            onClick={() =>
                              setShowProjectStatusDropdown(
                                !showProjectStatusDropdown
                              )
                            }
                            style={{
                            ...
                            >
                              {[
                                ...
                                <div
                                  key={option.value}
                                  onClick={() => {
                                    handleProjectStatusFilterChange(
                                      option.value
                                    );
                                    setShowProjectStatusDropdown(false);
                                  }}
                                  style={{
                                    ...
                                    fontWeight: "400",
                                    backgroundColor:
                                      projectStatusFilter === option.value
                                        ? "#f9fafb"
                                        : "#ffffff",
            ...
            {projectsLoading ? (
              ...
              <div>
                ...
                {customerProjectsLoading ? (
                  ...
                  <div style={{ textAlign: "center", padding: "40px" }}>
                    <div style={{ fontSize: "16px", color: "#6b7280" }}>
                      {(() => {
                        if (projectSearchTerm.trim()) {
                          return `No projects found matching "${projectSearchTerm}" for ${
                            selectedCustomer?.name || selectedCustomer
                          }.`;
                        } else if (projectStatusFilter === "all") {
                          return `No projects found for ${
                            selectedCustomer?.name || selectedCustomer
                          }.`;
                        } else {
                          return `No ${projectStatusFilter} projects found for ${
                            selectedCustomer?.name || selectedCustomer
                          }.`;
                        }
                      })()}
                ...
                {(() => {
                  const shouldShow =
                    !customerProjectsLoading && customerProjects.length > 0;
                        ...
                        return (
                          <div
                            key={
                              typeof customer === "string"
                                ? customer
                                : customer.id || customer._id || index
                            }
                            onClick={() =>
                              handleCustomerClick({ name: displayName })
                            }
                            style={{
                              backgroundColor: "#f8fafc",
                              borderRadius: "12px",
                    ...
                    !projectSearchTerm.trim() && (
                      <div style={{ marginTop: "20px" }}>
                        <SimplePagination
                          ...
                          totalPages={Math.max(
                            1,
                            Math.ceil(customersTotal / customersPageSize)
                          )}
              ...
              <div style={{ textAlign: "center", padding: "40px" }}>
                <div style={{ fontSize: "16px", color: "#6b7280" }}>
                  {projectSearchTerm.trim()
                    ? `No customers found matching "${projectSearchTerm}".`
                    : 'No customers found. Click "Create Project" to add your first project.'}
                </div>
              </div>
            )}
          </div>
              ...
              >
                ...
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <input
                    ...
                    required
                  />
                  {(newProjectDateErrors.start ||
                    newProjectDateErrors.range) && (
                    <div
                      style={{
                        color: "#dc2626",
                        fontSize: "12px",
          ...
          >
            ...
            >
              ...
              >
                ...
                <div style={{ marginBottom: "16px" }}>
                  <label
                    htmlFor="edit-project-customer-name"
                    style={{
                      display: "block",
                      marginBottom: "4px",
                      fontSize: "14px",
              ...
              >
                <div style={{ marginBottom: "16px" }}>
                  ...
                  >
                    ...
                    <option value="Computer Won't Start">
                    ...
                    <option value="Software Not Working">
                    ...
                    <option value="File Access Issues">
                      File Access Issues
                    </option>
                    <option value="Network Drive Problems">
                      Network Drive Problems
                    </option>
                    <option value="VPN Connection">VPN Connection</option>
                    <option value="Remote Access Issues">
                    ...
                    <option value="Website Not Loading">
                      Website Not Loading
                    </option>
                    ...
                    <option value="Camera Not Working">
                      Camera Not Working
                    </option>
                    ...
                    <option value="Mobile Device Sync">
                      Mobile Device Sync
                    </option>
                    <option value="Application Crashes">
                      Application Crashes
                    </option>
                    ...
                    <option value="Configuration Problems">
                    ...
                    <option value="Cloud Service Problems">
                    ...
                    <option value="Web Application Problems">
                      Web Application Problems
                    </option>
                    <option value="Other Technical Issue">
                  ...
                  onClick={async () => {
                    try {
                      await handleUpdateTicket(
                        editingTicket.id || editingTicket._id,
...
const WrappedDigitalMarketingDepartment = () => (
  <MarketingDepartmentWrapper>
    <DigitalMarketingDepartment />
  </MarketingDepartmentWrapper>
);

export default WrappedDigitalMarketingDepartment;
